...
typedef struct{
    ...
    // PSS in time domain (useful part of OFDM chips excluding CP)
    float pss_samples_re[3][LIBLTE_PHY_N_SAMPS_PER_SYMB_30_72MHZ];
    float pss_samples_im[3][LIBLTE_PHY_N_SAMPS_PER_SYMB_30_72MHZ];
...
/*********************************************************************
    Name: liblte_phy_dl_gen_sync_seq

    Description: generate synchronisation sequences (PSS and SSS)

    Document Reference:
*********************************************************************/
LIBLTE_ERROR_ENUM liblte_phy_dl_gen_sync_seq(LIBLTE_PHY_STRUCT   *phy_struct);


/*********************************************************************
    Name: liblte_phy_dl_primary_synchronisation

    Description: advanced primary synchronisation method

    Document Reference:
*********************************************************************/
LIBLTE_ERROR_ENUM liblte_phy_dl_primary_synchronisation(LIBLTE_PHY_STRUCT               *phy_struct,
                                                        float                           *i_samps,
                                                        float                           *q_samps,
                                                        uint32                           N_pss,
                                                        uint32                          *N_id_2,
...
/*********************************************************************
    Name: liblte_phy_timing_tracking_mib

    Description: Tracks the timing offset through MIB

    Document Reference: N/A
*********************************************************************/
LIBLTE_ERROR_ENUM liblte_phy_timing_tracking_mib(LIBLTE_PHY_STRUCT 		*phy_struct,
										  float             		*i_samps,
										  float             		*q_samps,
										  uint32            		frame_start_idx,
										  uint32            		N_id_cell,
										  ...
										  int8 		    			MAX_TRACKING_TIMING_DELAY_MIB,
										  int8	  	    			*timing_offset);

/*********************************************************************
    Name: liblte_phy_check_mib_decoding

    Description: Checks the correct decoding of MIB

    Document Reference: N/A
*********************************************************************/
LIBLTE_ERROR_ENUM liblte_phy_check_mib_decoding(LIBLTE_PHY_STRUCT 		*phy_struct,
                           	   		float             		*i_samps,
                                   		float             		*q_samps,
                             	   		uint32            		frame_start_idx,
                             	   		uint32            		N_id_cell,
...
/*********************************************************************
    Name: liblte_phy_tracking_freq_offset

    Description: Tracks the frequency offset

    Document Reference: 3GPP TS 36.211 v10.1.0
*********************************************************************/
LIBLTE_ERROR_ENUM liblte_phy_frequency_tracking_cp(LIBLTE_PHY_STRUCT               *phy_struct,
                                                  float                           *i_samps,
                                                  float                           *q_samps,
                                    	          uint32            	          frame_start_idx,
                                                  float             	          *freq_offset);
...
/*********************************************************************
    Name: liblte_phy_timing_tracking_rs

    Description: Tracks the timing offset through RS

    Document Reference: N/A
*********************************************************************/
LIBLTE_ERROR_ENUM liblte_phy_timing_tracking_rs(LIBLTE_PHY_STRUCT          *phy_struct,
												float                      *i_samps,
												float                      *q_samps,
												uint32                      frame_start_idx,
												uint32                      N_id_cell,
...
/*********************************************************************
    Name: liblte_phy_timing_frequency_rs

    Description: Tracks the frequency offset through RS

    Document Reference: N/A
*********************************************************************/
LIBLTE_ERROR_ENUM liblte_phy_frequency_tracking_rs(LIBLTE_PHY_STRUCT       *phy_struct,
												   float                   *i_samps,
												   float                   *q_samps,
												   uint32                   frame_start_idx,
												   uint32                   N_id_cell,
												   uint8                    N_ant,
												   LIBLTE_PHY_SUBFRAME_STRUCT *subframe,
												   float 		    		MAX_TRACKING_STEP_FREQUENCY_RS,
												   float             		*freq_offset);


/*********************************************************************
    Name: liblte_phy_rs_ce

    Description: Performs the channel estimation through RS

    Document Reference: 3GPP TS 36.211 v10.1.0
*********************************************************************/
LIBLTE_ERROR_ENUM liblte_phy_rs_ce(uint32            	       sfn,
...
/*********************************************************************
    Name: liblte_phy_pss_ce

    Description: Performs the channel estimation through PSS

    Document Reference: N/A
*********************************************************************/
LIBLTE_ERROR_ENUM liblte_phy_pss_ce(uint32             sfn,
				    LIBLTE_PHY_STRUCT *phy_struct,
                                    float             *i_samps,
                                    float             *q_samps,
                                    uint32             N_id_2,
...
/*********************************************************************
    Name: liblte_phy_sss_ce

    Description: Performs the channel estimation through SSS

    Document Reference: N/A
*********************************************************************/
LIBLTE_ERROR_ENUM liblte_phy_sss_ce(uint32             sfn,
				    LIBLTE_PHY_STRUCT *phy_struct,
                                    float             *i_samps,
                                    float             *q_samps,
                                    uint32             N_id_1,
...
/*********************************************************************
    Name: liblte_phy_channel_extender

    Description: Maximizes the number of resource blocks to perform 
		 channel estimation

    Document Reference: N/A
*********************************************************************/
void liblte_phy_channel_extender(LIBLTE_PHY_STRUCT *phy_struct, 
...
/*********************************************************************
    ...
    Description: add carrier frequency offset (CFO)
                 exp(j*2*pi*cfo*t) to I/Q samples
                 i_buf+j*q_buf to produce output i_rx_buf+j*q_rx_buf

    Document Reference: N/A
*********************************************************************/
void add_cfo(uint32 start_idx,
             uint32 num_samps,
             float *i_buf,
             float *q_buf,
             float fs,
...
/*********************************************************************
    ...
    NB: for single OFDM symbol with any CP length, CP can be zero
        to obtain the useful OFDM samples/chips

    Document Reference: 3GPP TS 36.211
*********************************************************************/
...
/*********************************************************************
    ...
    NB: for single OFDM symbol with any CP length

    Document Reference: 3GPP TS 36.211 section 6.12
*********************************************************************/
void samples_to_symbols(LIBLTE_PHY_STRUCT *phy_struct,
                        float             *samps_re,
                        float             *samps_im,
                        uint32             symbol_start_idx,
                        uint32             CP_len,
                        float             *symb_re,
                        float             *symb_im);
...
/*********************************************************************
    Name: liblte_phy_dl_primary_synch_xcorr
    Description: obtain integral CFO, PSS timing and N_id_2, able to handle
                 very large CFOs
    Input: N_pss: number of PSS OFDM symbols used for searching/training
    Output: N_id_2: part of cell ID
            est_int_cfo: estimated integral CFO
            useful_pss_timing: timing of useful part of a PSS OFDM symbol

    Document Reference: N/A
*********************************************************************/

void liblte_phy_dl_primary_synch_xcorr(LIBLTE_PHY_STRUCT               *phy_struct,
                                       float                           *i_samps,
                                       float                           *q_samps,
                                       float                           *i_samps_cfo,
                                       float                           *q_samps_cfo,
                                       ...
                                       uint32                          *N_id_2,
                                       int32                           *est_int_cfo,
                                       uint32                          *useful_pss_timing);


/*********************************************************************
    Name: liblte_phy_dl_exhaust_frac_cfo_estimator

    Description: exhaustive search the fractional CFO based on obtained
                 PSS timing, N_id_2 and integral CFO, then output the final CFO
                 estimate
    Input: N_pss: number of PSS OFDM symbols used for searching/training,
                  it need to be trade-off between computational complexity
                  and wireless performance
           N_id_2: estimated N_id_2
           est_int_cfo: estimated integral CFO
           useful_pss_timing: timing of useful part of a PSS OFDM symbol
    Output: est_cfo: final estimated integral CFO + fractional CFO

    Document Reference: N/A
*********************************************************************/
void liblte_phy_dl_exhaust_frac_cfo_estimator(LIBLTE_PHY_STRUCT               *phy_struct,
                                              float                           *i_samps,
                                              float                           *q_samps,
                                              float                           *i_samps_cfo,
                                              float                           *q_samps_cfo,
