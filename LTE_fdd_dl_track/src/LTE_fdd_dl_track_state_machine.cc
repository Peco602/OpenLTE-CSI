...
*******************************************************************************
    ...
    File: LTE_fdd_dl_track_state_machine.cc
...
#define SAMP_BUF_SIZE         (307200*1) //*20
...
LTE_fdd_dl_track_state_machine_sptr LTE_fdd_dl_track_make_state_machine(uint32 samp_rate)
{
    return LTE_fdd_dl_track_state_machine_sptr(new LTE_fdd_dl_track_state_machine(samp_rate));
}

LTE_fdd_dl_track_state_machine::LTE_fdd_dl_track_state_machine(uint32 samp_rate)
    : gr::sync_block ("LTE_fdd_dl_track_state_machine",
...
LTE_fdd_dl_track_state_machine::~LTE_fdd_dl_track_state_machine()
{
    // Print tracking results
    LTE_fdd_dl_track_interface  *closing_interface = LTE_fdd_dl_track_interface::get_instance();
    std::string          closing_msg;

    printf("\n***************************** TRACKING COMPLETED *****************************\n");
    if (total_frames>0) {
	float quality = (float)(total_frames - lost_frames)/total_frames*100;
	closing_msg  = ">>\tCellID ";
	closing_msg += boost::lexical_cast<std::string>(chan_data.N_id_cell);
	closing_msg += "\t\tFrames ";
	closing_msg += boost::lexical_cast<std::string>(total_frames);
	closing_msg += "\t\tMissing ";
	closing_msg += boost::lexical_cast<std::string>(lost_frames);
	closing_msg += "\t\tCorrect ";
	closing_msg += boost::lexical_cast<std::string>(total_frames-lost_frames);
	closing_msg += "\t\tQuality ";
	closing_msg += boost::lexical_cast<std::string>(quality);
	closing_msg += "%";
    } else {
	closing_msg = ">>\tNo LTE cells found";
    }
    std::cout << closing_msg + "\n";
    closing_interface->send_ctrl_msg(closing_msg);
    printf("********************************************************************************\n");
...
int32 LTE_fdd_dl_track_state_machine::work(int32                      ninput_items,
                                          ...
                                          gr_vector_void_star       &output_items)
{
    ...
    if(process_samples)
    {
        if(LTE_FDD_DL_TRACK_STATE_MACHINE_STATE_COARSE_TIMING_SEARCH != state)
        {
 	       ...
 	       if(LIBLTE_SUCCESS == liblte_phy_dl_find_coarse_timing_and_freq_offset(phy_struct,
                                                                                      ...
                                                                                      &timing_struct))
                {
                    if(corr_peak_idx < timing_struct.n_corr_peaks)
                    {
                        ...
                        state          = LTE_FDD_DL_TRACK_STATE_MACHINE_STATE_PSS_AND_FINE_TIMING_SEARCH;
		...
		libgraph_print_screen(state,NULL,0);
                                                                         ...
                                                                         &freq_offset))
                {
                    ...
                    // Search for SSS
                    state          = LTE_FDD_DL_TRACK_STATE_MACHINE_STATE_SSS_SEARCH;
                    N_samps_needed = sss_search_num_samps;
                }else{
                    // Go back to coarse timing search
                    state          = LTE_FDD_DL_TRACK_STATE_MACHINE_STATE_COARSE_TIMING_SEARCH;
                    samp_buf_r_idx = 0;
                    samp_buf_w_idx = 0;
                    N_samps_needed = coarse_timing_search_num_samps;
                }
		...
		libgraph_print_screen(state,NULL,0);
                if(LIBLTE_SUCCESS == liblte_phy_find_sss(phy_struct,
                                                         ...
                                                         &frame_start_idx))
                {
                    if(interface->is_banned_cell(3*N_id_1 + N_id_2) || !interface->is_forced_cell(3*N_id_1 + N_id_2))
                    {
                        // Go back to coarse timing search
                    	state          = LTE_FDD_DL_TRACK_STATE_MACHINE_STATE_COARSE_TIMING_SEARCH;
                    	samp_buf_r_idx = 0;
                        ...
                        // Decode BCH
                        state = LTE_FDD_DL_TRACK_STATE_MACHINE_STATE_BCH_DECODE;
		...
		libgraph_print_screen(state,NULL,0);
		    ...
		    libgraph_print_screen(state,sfn,chan_data.N_id_cell,phy_struct->N_rb_dl,N_ant,mib.dl_bw,0,0,0,NULL,total_frames,lost_frames,(char *)"[+] MIB correctly decoded",0);
                    ...
                    //{
                        ...
                        state = LTE_FDD_DL_TRACK_STATE_MACHINE_STATE_PDSCH_DECODE_SIB1;
                        if((sfn % 2) != 0)
                        {
		...
		libgraph_print_screen(state,sfn,chan_data.N_id_cell,phy_struct->N_rb_dl,N_ant,mib.dl_bw,0,0,0,NULL,total_frames,lost_frames,NULL,0);
                if(LIBLTE_SUCCESS == liblte_phy_get_dl_subframe_and_ce(phy_struct,
                                                                       i_buf,
                                                                       q_buf,
                                                                       samp_buf_r_idx,
                                                                      ...
                                                                      &bcch_dlsch_msg))
                {
                    ...
                    // Decode all PDSCHs
                    // state          = LTE_FDD_DL_TRACK_STATE_MACHINE_STATE_PDSCH_DECODE_SI_GENERIC;
                    // N_sfr          = 0;
                    // N_samps_needed = pdsch_decode_si_generic_num_samps;

		    // Goto CHANNEL ESTIMATION STATE
		    state            = LTE_FDD_DL_TRACK_STATE_MACHINE_STATE_CHANNEL_ESTIMATION;
		    liblte_phy_channel_extender(phy_struct,mib.dl_bw);
		    //N_sfr             = 0;
		    samp_buf_r_idx += one_frame_num_samps;
		    N_samps_needed  = one_frame_num_samps;
		...
		TRACKING = liblte_phy_timing_tracking_rs(phy_struct,i_buf,q_buf,samp_buf_r_idx,chan_data.N_id_cell,N_ant,&subframe,2,&timing_offset,sfn_offset,rrc_msg,mib);
		if(TRACKING != LIBLTE_SUCCESS)
        {
			TRACKING = liblte_phy_timing_tracking_mib(phy_struct,i_buf,q_buf,samp_buf_r_idx,chan_data.N_id_cell,N_ant,sfn_offset,rrc_msg,mib,2,&timing_offset);
		}
		samp_buf_r_idx += timing_offset;
		TIMING_OFFSET += (float)timing_offset*1000000000/sampling_rate;

		// Frequency offsets tracking & correction
		//liblte_phy_frequency_tracking_rs(phy_struct,i_buf,q_buf,samp_buf_r_idx,N_id_cell,N_ant,&subframe,10,&freq_offset);
		liblte_phy_frequency_tracking_cp(phy_struct,i_buf,q_buf,samp_buf_r_idx,&freq_offset);
		freq_shift(samp_buf_r_idx,one_frame_num_samps, freq_offset);
        FREQUENCY_OFFSET = timing_struct.freq_offset[corr_peak_idx] + freq_offset;
		
		if(TRACKING == LIBLTE_SUCCESS)
        {		
			// PSS Channel Estimation
			//liblte_phy_pss_ce(sfn,phy_struct,i_buf,q_buf,N_id_2,samp_buf_r_idx,chan_data.N_id_cell);

			// SSS Channel Estimation	
			//liblte_phy_sss_ce(sfn,phy_struct,i_buf,q_buf,N_id_1,N_id_2,samp_buf_r_idx,chan_data.N_id_cell);

			// RS Channel Estimation
			liblte_phy_rs_ce(sfn,phy_struct,i_buf,q_buf,samp_buf_r_idx,chan_data.N_id_cell,N_ant,&ant_power[0],&subframe);
		} else {
			lost_frames++;
			lost_frames_health++;	
		}				
		...
		if (sfn % HEALTH_UPDATE == 0) {
			...
			total_frames_health = 0;
			lost_frames_health  = 0;
			libgraph_print_screen(state,sfn,chan_data.N_id_cell,phy_struct->N_rb_dl,N_ant,mib.dl_bw,HEALTH,TIMING_OFFSET,FREQUENCY_OFFSET,&ant_power[0],total_frames,lost_frames,NULL,0);
		}

		if (HEALTH <= HEALTH_THRESHOLD) {
            // Go back to coarse timing search
			libgraph_print_screen(state,sfn,chan_data.N_id_cell,phy_struct->N_rb_dl,N_ant,mib.dl_bw,HEALTH,TIMING_OFFSET,FREQUENCY_OFFSET,&ant_power[0],total_frames,lost_frames,(char *)"[!] Signal Health too low - Going back to COARSE SEARCH",3);
			sfn 			= 0;
			HEALTH 			= 100;
			TIMING_OFFSET 	   	= 0;
			FREQUENCY_OFFSET     	= 0;
...
void LTE_fdd_dl_track_state_machine::init(void)
{
    state                 = LTE_FDD_DL_TRACK_STATE_MACHINE_STATE_COARSE_TIMING_SEARCH;
    phich_res             = 0;
    sfn                   = 0;
...
void LTE_fdd_dl_track_state_machine::copy_input_to_samp_buf(const gr_complex *in, int32 ninput_items)
{
    uint32 i;
...
void LTE_fdd_dl_track_state_machine::freq_shift(uint32 start_idx, uint32 num_samps, float freq_offset)
{
    float  f_samp_re;
    float  f_samp_im;
    float  tmp_i;
...
void LTE_fdd_dl_track_state_machine::channel_found(bool  &switch_freq,
                                                  int32 &done_flag)
{
    LTE_fdd_dl_track_interface *interface = LTE_fdd_dl_track_interface::get_instance();
    ...
    if(corr_peak_idx >= timing_struct.n_corr_peaks)
    {
        // Change frequency
        corr_peak_idx = 0;
        if(LTE_FDD_DL_TRACK_STATUS_OK == interface->switch_to_next_freq())
        {
            switch_freq = true;
            send_cnf    = true;
        }else{
...
void LTE_fdd_dl_track_state_machine::channel_not_found(bool  &switch_freq,
                                                      int32 &done_flag)
{
    LTE_fdd_dl_track_interface *interface = LTE_fdd_dl_track_interface::get_instance();
