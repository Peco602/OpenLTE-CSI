...
*******************************************************************************
    ...
    File: LTE_fdd_dl_track_interface.cc
...
LTE_fdd_dl_track_interface* LTE_fdd_dl_track_interface::instance       = NULL;
...
bool                       LTE_fdd_dl_track_interface::ctrl_connected = false;
...
// Singleton
LTE_fdd_dl_track_interface* LTE_fdd_dl_track_interface::get_instance(void)
{
    libtools_scoped_lock lock(interface_instance_mutex);

    if(NULL == instance)
    {
        instance = new LTE_fdd_dl_track_interface();
    }

    return(instance);
}
void LTE_fdd_dl_track_interface::cleanup(void)
{
    libtools_scoped_lock lock(interface_instance_mutex);
...
LTE_fdd_dl_track_interface::LTE_fdd_dl_track_interface()
{
    uint32 i;
    ...
    ctrl_socket    = NULL;
    ctrl_port      = LTE_FDD_DL_TRACK_DEFAULT_CTRL_PORT;
    ctrl_connected = false;

    // Variables
    pthread_mutex_init(&dl_earfcn_list_mutex, NULL);
    band                = LIBLTE_INTERFACE_BAND_20;
...
// Communication
void LTE_fdd_dl_track_interface::set_ctrl_port(int16 port)
{
...
void LTE_fdd_dl_track_interface::start_ctrl_port(void)
{
...
void LTE_fdd_dl_track_interface::stop_ctrl_port(void)
{
    libtools_scoped_lock lock(ctrl_mutex);
...
void LTE_fdd_dl_track_interface::send_ctrl_msg(std::string msg)
{
    libtools_scoped_lock lock(connect_mutex);
    std::string          tmp_msg;
...
void LTE_fdd_dl_track_interface::send_ctrl_info_msg(std::string msg)
{
    libtools_scoped_lock lock(connect_mutex);
    std::string          tmp_msg;
...
void LTE_fdd_dl_track_interface::send_ctrl_channel_found_begin_msg(LTE_FDD_DL_TRACK_CHAN_DATA_STRUCT *chan_data,
...
void LTE_fdd_dl_track_interface::send_ctrl_sib1_decoded_msg(LTE_FDD_DL_TRACK_CHAN_DATA_STRUCT        *chan_data,
...
void LTE_fdd_dl_track_interface::send_ctrl_sib2_decoded_msg(LTE_FDD_DL_TRACK_CHAN_DATA_STRUCT        *chan_data,
...
void LTE_fdd_dl_track_interface::send_ctrl_sib3_decoded_msg(LTE_FDD_DL_TRACK_CHAN_DATA_STRUCT        *chan_data,
...
void LTE_fdd_dl_track_interface::send_ctrl_sib4_decoded_msg(LTE_FDD_DL_TRACK_CHAN_DATA_STRUCT        *chan_data,
...
void LTE_fdd_dl_track_interface::send_ctrl_sib5_decoded_msg(LTE_FDD_DL_TRACK_CHAN_DATA_STRUCT        *chan_data,
...
void LTE_fdd_dl_track_interface::send_ctrl_sib6_decoded_msg(LTE_FDD_DL_TRACK_CHAN_DATA_STRUCT        *chan_data,
...
void LTE_fdd_dl_track_interface::send_ctrl_sib7_decoded_msg(LTE_FDD_DL_TRACK_CHAN_DATA_STRUCT        *chan_data,
...
void LTE_fdd_dl_track_interface::send_ctrl_sib8_decoded_msg(LTE_FDD_DL_TRACK_CHAN_DATA_STRUCT        *chan_data,
...
void LTE_fdd_dl_track_interface::send_ctrl_sib13_decoded_msg(LTE_FDD_DL_TRACK_CHAN_DATA_STRUCT         *chan_data,
...
void LTE_fdd_dl_track_interface::send_ctrl_channel_found_end_msg(LTE_FDD_DL_TRACK_CHAN_DATA_STRUCT *chan_data)
{
    libtools_scoped_lock lock(connect_mutex);
    std::string          tmp_msg;
...
void LTE_fdd_dl_track_interface::send_ctrl_channel_not_found_msg(void)
{
    libtools_scoped_lock lock(connect_mutex);
    std::string          tmp_msg;
...
void LTE_fdd_dl_track_interface::send_ctrl_status_msg(LTE_FDD_DL_TRACK_STATUS_ENUM status,
                                                     std::string                 msg)
{
    libtools_scoped_lock lock(connect_mutex);
    std::string          tmp_msg;
    ...
    if(ctrl_connected)
    {
        if(LTE_FDD_DL_TRACK_STATUS_OK == status)
        {
            tmp_msg = "ok ";
        }else{
            tmp_msg = "fail ";
...
void LTE_fdd_dl_track_interface::open_pcap_fd(void)
{
    ...
    pcap_fd = fopen("/tmp/LTE_fdd_dl_track.pcap", "w");
...
void LTE_fdd_dl_track_interface::send_pcap_msg(uint32                 rnti,
                                              uint32                 current_tti,
...
void LTE_fdd_dl_track_interface::handle_ctrl_msg(std::string msg)
{
    ...
    if(std::string::npos != msg.find("read"))
    {
        ...
        interface->send_ctrl_status_msg(LTE_FDD_DL_TRACK_STATUS_OK, "");
    }else if(std::string::npos != msg.find("help")){
        interface->handle_help();
    }else{
        interface->send_ctrl_status_msg(LTE_FDD_DL_TRACK_STATUS_FAIL, "Invalid param");
    }
}
void LTE_fdd_dl_track_interface::handle_ctrl_connect(void)
{
    LTE_fdd_dl_track_interface *interface = LTE_fdd_dl_track_interface::get_instance();

    pthread_mutex_lock(&connect_mutex);
    LTE_fdd_dl_track_interface::ctrl_connected = true;
    pthread_mutex_unlock(&connect_mutex);

    interface->send_ctrl_msg("*** LTE FDD DL TRACK ***");
    interface->send_ctrl_msg("Type help to see a list of commands");
}
void LTE_fdd_dl_track_interface::handle_ctrl_disconnect(void)
{
    libtools_scoped_lock lock(connect_mutex);

    LTE_fdd_dl_track_interface::ctrl_connected = false;
}
void LTE_fdd_dl_track_interface::handle_ctrl_error(LIBTOOLS_SOCKET_WRAP_ERROR_ENUM err)
{
...
// Handlers
void LTE_fdd_dl_track_interface::handle_read(std::string msg)
{
    if(std::string::npos != msg.find(BAND_PARAM))
    {
        ...
        read_gain();
    }else if(std::string::npos != msg.find(BANNED_CELLS_PARAM)){
        read_banned_cells();
    }else if(std::string::npos != msg.find(FORCED_CELLS_PARAM)){
        read_forced_cells();
    }else{
        send_ctrl_status_msg(LTE_FDD_DL_TRACK_STATUS_FAIL, "Invalid read");
    }
}
void LTE_fdd_dl_track_interface::handle_write(std::string msg)
{
    if(std::string::npos != msg.find(BAND_PARAM))
    {
        ...
        write_banned_cells(msg.substr(msg.find(BANNED_CELLS_PARAM)+sizeof(BANNED_CELLS_PARAM), std::string::npos).c_str());
    }else if(std::string::npos != msg.find(FORCED_CELLS_PARAM)){
        write_forced_cells(msg.substr(msg.find(FORCED_CELLS_PARAM)+sizeof(FORCED_CELLS_PARAM), std::string::npos).c_str());
    }else if(std::string::npos != msg.find(GAIN_PARAM)){
        write_gain(msg.substr(msg.find(GAIN_PARAM)+sizeof(GAIN_PARAM), std::string::npos).c_str());
    }else{
        send_ctrl_status_msg(LTE_FDD_DL_TRACK_STATUS_FAIL, "Invalid write");
    }
}
void LTE_fdd_dl_track_interface::handle_start(void)
{
    ...
    if(!flowgraph->is_started())
    {
        dl_earfcn_list_idx = 0;
        current_dl_earfcn  = dl_earfcn_list[dl_earfcn_list_idx];
        if(LTE_FDD_DL_TRACK_STATUS_OK == flowgraph->start(current_dl_earfcn))
        {
            send_ctrl_status_msg(LTE_FDD_DL_TRACK_STATUS_OK, "");
        }else{
            send_ctrl_status_msg(LTE_FDD_DL_TRACK_STATUS_FAIL, "Start fail, likely there is no hardware connected");
        }
    }else{
        send_ctrl_status_msg(LTE_FDD_DL_TRACK_STATUS_FAIL, "Flowgraph already started");
    }
}
void LTE_fdd_dl_track_interface::handle_stop(void)
{
    LTE_fdd_dl_track_flowgraph *flowgraph = LTE_fdd_dl_track_flowgraph::get_instance();

    if(flowgraph->is_started())
    {
        if(LTE_FDD_DL_TRACK_STATUS_OK == flowgraph->stop())
        {
            send_ctrl_status_msg(LTE_FDD_DL_TRACK_STATUS_OK, "");
        }else{
            send_ctrl_status_msg(LTE_FDD_DL_TRACK_STATUS_FAIL, "Stop fail");
        }
    }else{
        send_ctrl_status_msg(LTE_FDD_DL_TRACK_STATUS_FAIL, "Flowgraph not started");
    }
}
void LTE_fdd_dl_track_interface::handle_help(void)
{
    libtools_scoped_lock lock(dl_earfcn_list_mutex);
    std::string          tmp_str;
    uint32               i;
    ...
    // Banned Cells
    tmp_str  = "\t\t";
    tmp_str += BANNED_CELLS_PARAM;
    tmp_str += " = ";
    try
    {
        for(i=0; i<n_banned_cells; i++)
        {
            tmp_str += boost::lexical_cast<std::string>(banned_cells[i]);
            if(i != n_banned_cells-1)
            {
                tmp_str += ",";
            }
        }
    ...
    // Forced Cells
    tmp_str  = "\t\t";
    tmp_str += FORCED_CELLS_PARAM;
    tmp_str += " = ";
    try
    {
        for(i=0; i<n_forced_cells; i++)
        {
            tmp_str += boost::lexical_cast<std::string>(forced_cells[i]);
            if(i != n_forced_cells-1)
            {
                tmp_str += ",";
            }
        }
...
// Gets/Sets
bool LTE_fdd_dl_track_interface::get_shutdown(void)
{
    return(shutdown);
}
int16 LTE_fdd_dl_track_interface::get_ppm_corr(void)
{
    return(ppm_corr);
}
uint32 LTE_fdd_dl_track_interface::get_gain(void)
{
    return(gain);
}

bool LTE_fdd_dl_track_interface::is_banned_cell(uint32 cell_to_verify)
{
    bool banned = false;

    for (int i=0; i<n_banned_cells; i++) {
	if (banned_cells[i] == cell_to_verify) {
	   banned = true;
	   break;
	}
    }
...
bool LTE_fdd_dl_track_interface::is_forced_cell(uint32 cell_to_verify)
{
    if (n_forced_cells == 0)
    {
	return(true);
    ...
    for (int i=0; i<n_forced_cells; i++) {
	if (forced_cells[i] == cell_to_verify) {
	   return(true);
	}
    }
...
// Reads/Writes
void LTE_fdd_dl_track_interface::read_band(void)
{
    send_ctrl_status_msg(LTE_FDD_DL_TRACK_STATUS_OK, liblte_interface_band_text[band]);
}
void LTE_fdd_dl_track_interface::write_band(std::string band_str)
{
    libtools_scoped_lock lock(dl_earfcn_list_mutex);
    uint32               i;
    ...
    {
        send_ctrl_status_msg(LTE_FDD_DL_TRACK_STATUS_FAIL, "Invalid Band");
    }else{
        dl_earfcn_list_size = liblte_interface_last_dl_earfcn[band] - liblte_interface_first_dl_earfcn[band] + 1;
        for(i=0; i<dl_earfcn_list_size; i++)
        {
...
void LTE_fdd_dl_track_interface::read_dl_earfcn_list(void)
{
    libtools_scoped_lock lock(dl_earfcn_list_mutex);
    std::string          tmp_str;
    uint32               i;
    ...
    try
    {
        ...
        send_ctrl_status_msg(LTE_FDD_DL_TRACK_STATUS_OK, tmp_str);
    }catch(boost::bad_lexical_cast &){
        send_ctrl_status_msg(LTE_FDD_DL_TRACK_STATUS_FAIL, "bad dl_earfcn_list");
    }
}
void LTE_fdd_dl_track_interface::write_dl_earfcn_list(std::string dl_earfcn_list_str)
{
    libtools_scoped_lock        lock(dl_earfcn_list_mutex);
    LTE_FDD_DL_TRACK_STATUS_ENUM stat = LTE_FDD_DL_TRACK_STATUS_OK;
    uint32                      i;
    ...
    while(std::string::npos != dl_earfcn_list_str.find(","))
    {
        try
        {
            ...
            stat = LTE_FDD_DL_TRACK_STATUS_FAIL;
            break;
        }
    ...
    if(LTE_FDD_DL_TRACK_STATUS_FAIL == stat)
    {
...
void LTE_fdd_dl_track_interface::read_enable_pcap(void)
{
    if(true == enable_pcap)
    {
        send_ctrl_status_msg(LTE_FDD_DL_TRACK_STATUS_OK, "on");
    }else{
        send_ctrl_status_msg(LTE_FDD_DL_TRACK_STATUS_OK, "off");
    }
}
void LTE_fdd_dl_track_interface::write_enable_pcap(std::string enable_pcap_str)
{
    if(enable_pcap_str == "on")
    {
        ...
        send_ctrl_status_msg(LTE_FDD_DL_TRACK_STATUS_FAIL, "Invalid enable_pcap");
    }
}
void LTE_fdd_dl_track_interface::read_banned_cells(void)
{
    std::string          tmp_str;
    uint32               i;
    ...
    try
    {
        ...
        send_ctrl_status_msg(LTE_FDD_DL_TRACK_STATUS_FAIL, "bad banned_cells_list");
    }
}
void LTE_fdd_dl_track_interface::write_banned_cells(std::string banned_cells_str)
{
    LTE_FDD_DL_TRACK_STATUS_ENUM stat = LTE_FDD_DL_TRACK_STATUS_OK;
    uint32                      i;
    uint32                      tmp_list[65535];
    uint32                      tmp_list_size = 0;

    while(std::string::npos != banned_cells_str.find(","))
    {
        try
        {
            tmp_list[tmp_list_size] = boost::lexical_cast<uint32>(banned_cells_str.substr(0, banned_cells_str.find(",")));
        }catch(boost::bad_lexical_cast &){
            stat = LTE_FDD_DL_TRACK_STATUS_FAIL;
            break;
        }
        if(tmp_list[tmp_list_size] >= 0 &&
           tmp_list[tmp_list_size] <= 268435455)
        {
            tmp_list_size++;
        }else{
            stat = LTE_FDD_DL_TRACK_STATUS_FAIL;
        ...
        banned_cells_str = banned_cells_str.substr(banned_cells_str.find(",")+1, std::string::npos);
    }
    try
    {
        tmp_list[tmp_list_size] = boost::lexical_cast<uint32>(banned_cells_str);
    }catch(boost::bad_lexical_cast &){
        stat = LTE_FDD_DL_TRACK_STATUS_FAIL;
    }
    if(tmp_list[tmp_list_size] >= 0 &&
    ...
    if(LTE_FDD_DL_TRACK_STATUS_FAIL == stat)
    {
        send_ctrl_status_msg(stat, "Invalid banned_cells_list");
    }else{
        for(i=0; i<tmp_list_size; i++)
        {
            banned_cells[i] = tmp_list[i];
        }
        n_banned_cells = tmp_list_size;
        send_ctrl_status_msg(stat, "");
    }
}
void LTE_fdd_dl_track_interface::read_forced_cells(void)
{
    std::string          tmp_str;
    uint32               i;
    ...
    try
    {
        ...
        send_ctrl_status_msg(LTE_FDD_DL_TRACK_STATUS_FAIL, "bad forced_cells_list");
    }
}
void LTE_fdd_dl_track_interface::write_forced_cells(std::string forced_cells_str)
{
    LTE_FDD_DL_TRACK_STATUS_ENUM stat = LTE_FDD_DL_TRACK_STATUS_OK;
    uint32                      i;
    uint32                      tmp_list[65535];
    ...
    while(std::string::npos != forced_cells_str.find(","))
    {
        try
        {
            tmp_list[tmp_list_size] = boost::lexical_cast<uint32>(forced_cells_str.substr(0, forced_cells_str.find(",")));
        }catch(boost::bad_lexical_cast &){
            stat = LTE_FDD_DL_TRACK_STATUS_FAIL;
            break;
        }
        ...
        forced_cells_str = forced_cells_str.substr(forced_cells_str.find(",")+1, std::string::npos);
    }
    try
    {
        tmp_list[tmp_list_size] = boost::lexical_cast<uint32>(forced_cells_str);
    }catch(boost::bad_lexical_cast &){
        stat = LTE_FDD_DL_TRACK_STATUS_FAIL;
    }
    if(tmp_list[tmp_list_size] >= 0 &&
    ...
    if(LTE_FDD_DL_TRACK_STATUS_FAIL == stat)
    {
        send_ctrl_status_msg(stat, "Invalid forced_cells_list");
    }else{
        for(i=0; i<tmp_list_size; i++)
        {
            forced_cells[i] = tmp_list[i];
        }
        n_forced_cells = tmp_list_size;
        send_ctrl_status_msg(stat, "");
    }
}
void LTE_fdd_dl_track_interface::read_ppm_corr(void)
{
    send_ctrl_status_msg(LTE_FDD_DL_TRACK_STATUS_OK, boost::lexical_cast<std::string>(ppm_corr));
}
void LTE_fdd_dl_track_interface::write_ppm_corr(std::string ppm_corr_str)
{
    ppm_corr = boost::lexical_cast<int16>(ppm_corr_str);
    send_ctrl_status_msg(LTE_FDD_DL_TRACK_STATUS_OK, "");
}
void LTE_fdd_dl_track_interface::read_gain(void)
{
    send_ctrl_status_msg(LTE_FDD_DL_TRACK_STATUS_OK, boost::lexical_cast<std::string>(gain));
}
void LTE_fdd_dl_track_interface::write_gain(std::string gain_str)
{
	...
	gain = 0;
    	send_ctrl_status_msg(LTE_FDD_DL_TRACK_STATUS_FAIL, "Invalid gain value");
    }
}

// Helpers
LTE_FDD_DL_TRACK_STATUS_ENUM LTE_fdd_dl_track_interface::switch_to_next_freq(void)
{
    libtools_scoped_lock         lock(dl_earfcn_list_mutex);
    LTE_fdd_dl_track_flowgraph   *flowgraph = LTE_fdd_dl_track_flowgraph::get_instance();
    LTE_FDD_DL_TRACK_STATUS_ENUM  stat      = LTE_FDD_DL_TRACK_STATUS_FAIL;
    ...
    if(dl_earfcn_list_idx < dl_earfcn_list_size)
    {
          ...
          stat = LTE_FDD_DL_TRACK_STATUS_OK;
    }
