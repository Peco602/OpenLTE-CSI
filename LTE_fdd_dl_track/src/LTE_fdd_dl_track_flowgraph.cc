...
*******************************************************************************
    ...
    File: LTE_fdd_dl_track_flowgraph.cc
...
LTE_fdd_dl_track_flowgraph* LTE_fdd_dl_track_flowgraph::instance = NULL;
...
// Singleton
LTE_fdd_dl_track_flowgraph* LTE_fdd_dl_track_flowgraph::get_instance(void)
{
    libtools_scoped_lock lock(flowgraph_instance_mutex);

    if(NULL == instance)
    {
        instance = new LTE_fdd_dl_track_flowgraph();
    }

    return(instance);
}
void LTE_fdd_dl_track_flowgraph::cleanup(void)
{
    libtools_scoped_lock lock(flowgraph_instance_mutex);
...
LTE_fdd_dl_track_flowgraph::LTE_fdd_dl_track_flowgraph()
{
...
// Flowgraph
bool LTE_fdd_dl_track_flowgraph::is_started(void)
{
    libtools_scoped_lock lock(start_mutex);

    return(started);
}
LTE_FDD_DL_TRACK_STATUS_ENUM LTE_fdd_dl_track_flowgraph::start(uint16 dl_earfcn)
{
    libtools_scoped_lock          lock(start_mutex);
    LTE_fdd_dl_track_interface    *interface = LTE_fdd_dl_track_interface::get_instance();
    uhd::device_addr_t            hint;
    LTE_FDD_DL_TRACK_STATUS_ENUM   err           = LTE_FDD_DL_TRACK_STATUS_FAIL;
    LTE_FDD_DL_TRACK_HW_TYPE_ENUM  hardware_type = LTE_FDD_DL_TRACK_HW_TYPE_UNKNOWN;
    double                        mcr;
    ...
    if(!started)
    {
        ...
        if(NULL == samp_src.get())
        {
            ...
            if(0 != tmp_src0->get_sample_rates().size() &&
               1 >= fabs(mcr - 15360000))
            {
                hardware_type = LTE_FDD_DL_TRACK_HW_TYPE_USRP_B;
                samp_src      = tmp_src0;
            }else{
                BOOST_FOREACH(const uhd::device_addr_t &dev, uhd::device::find(hint))
                {
                ...
                {
                    hardware_type = LTE_FDD_DL_TRACK_HW_TYPE_USRP_N;
                    samp_src      = tmp_src0;
                }else{
                    BOOST_FOREACH(const uhd::device_addr_t &dev, uhd::device::find(hint))
                    {
                        if(dev["type"] == "umtrx")
                        {
                            hardware_type = LTE_FDD_DL_TRACK_HW_TYPE_UMTRX;
                            samp_src      = tmp_src0;
                            break;
                        }
                    }
                    if(hardware_type == LTE_FDD_DL_TRACK_HW_TYPE_UNKNOWN)
                    {
                        tmp_src0.reset();
                        ...
                        BOOST_FOREACH(const uhd::device_addr_t &dev, uhd::device::find(hint))
                        {
                            if(dev["type"] == "x300")
                            {
                                hardware_type = LTE_FDD_DL_TRACK_HW_TYPE_USRP_X;
                                samp_src      = tmp_src0;
                                break;
                            }
                        }
        ...
        if(NULL == state_machine.get())
        {
            ...
            {
            case LTE_FDD_DL_TRACK_HW_TYPE_USRP_B:
                state_machine = LTE_fdd_dl_track_make_state_machine(15360000);
                break;
            case LTE_FDD_DL_TRACK_HW_TYPE_USRP_N:
                state_machine = LTE_fdd_dl_track_make_state_machine(15360000);
                break;
            case LTE_FDD_DL_TRACK_HW_TYPE_UMTRX:
                state_machine = LTE_fdd_dl_track_make_state_machine(7680000);
                break;
            case LTE_FDD_DL_TRACK_HW_TYPE_USRP_X:
                state_machine = LTE_fdd_dl_track_make_state_machine(15360000);
                break;
            case LTE_FDD_DL_TRACK_HW_TYPE_HACKRF:
                state_machine = LTE_fdd_dl_track_make_state_machine(15360000);
                break;
            case LTE_FDD_DL_TRACK_HW_TYPE_BLADERF:
                state_machine = LTE_fdd_dl_track_make_state_machine(15360000);
                break;
            case LTE_FDD_DL_TRACK_HW_TYPE_UNKNOWN:
            default:
                printf("Unknown hardware, treating like RTL-SDR\n");
            case LTE_FDD_DL_TRACK_HW_TYPE_RTL_SDR:
                state_machine = LTE_fdd_dl_track_make_state_machine(1920000);
                break;
            }
        }
           ...
           NULL != state_machine.get())
        {
		    ...
		    if (samp_src->get_gain()==0)
		    {
                    	samp_src->set_gain_mode(true);
		    } else {
                    	samp_src->set_gain_mode(false);
			samp_src->set_gain(samp_src->get_gain());
		    }
		    samp_src->set_freq_corr(interface->get_ppm_corr(),0);
                    break;
                }
                samp_src->set_center_freq(liblte_interface_dl_earfcn_to_frequency(dl_earfcn));

                if(LTE_FDD_DL_TRACK_HW_TYPE_USRP_N == hardware_type)
                {
                ...
                }else if(LTE_FDD_DL_TRACK_HW_TYPE_UMTRX == hardware_type){
                ...
                {
                    err     = LTE_FDD_DL_TRACK_STATUS_OK;
                    started = true;
                }else{
...
LTE_FDD_DL_TRACK_STATUS_ENUM LTE_fdd_dl_track_flowgraph::stop(void)
{
    libtools_scoped_lock        lock(start_mutex);
    LTE_FDD_DL_TRACK_STATUS_ENUM err = LTE_FDD_DL_TRACK_STATUS_FAIL;
...
void LTE_fdd_dl_track_flowgraph::update_center_freq(uint16 dl_earfcn)
{
    libtools_scoped_lock lock(start_mutex);
...
// Run
void* LTE_fdd_dl_track_flowgraph::run_thread(void *inputs)
{
    LTE_fdd_dl_track_interface *interface = LTE_fdd_dl_track_interface::get_instance();
    LTE_fdd_dl_track_flowgraph *flowgraph = (LTE_fdd_dl_track_flowgraph *)inputs;
