...
then
	echo ""
    	echo "Usage: ./importEvrytania_RTL [options]" 
	echo ""
	echo "	-f, --inputfolder           Folder with LTE data [./INPUT_DATA]"
	echo "	-h, --help	            Help"
	echo ""
	echo "Examples:"
	echo "./importEvrytania_RTL.sh -f ./RTL_DATA 	==>  Acquire RTL LTE data from ./RTL_DATA"
	echo ""
    	exit
fi	
...
do	
	...
	echo ../../$source_path
	#LTE-Tracker -f $frequency -s -l ../../$source_path > /dev/null
	####################################
	tracked=0
	drop=0 # 0
	delta=0.5 # 0.5
	...
	do
		...
		timeout --foreground -k $timeout $timeout LTE-Tracker -f $frequency -s -l ../../$source_path -d $drop  #> /dev/null
		if [[ $? -ne 124 ]]
		then
			tracked=1
			break
