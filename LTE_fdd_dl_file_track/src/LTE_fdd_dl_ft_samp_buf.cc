...
#define HEALTH_UPDATE				 (30)
#define HEALTH_THRESHOLD			 (10)
...
LTE_fdd_dl_ft_samp_buf_sptr LTE_fdd_dl_ft_make_samp_buf(size_t in_size_val)
{
    return LTE_fdd_dl_ft_samp_buf_sptr(new LTE_fdd_dl_ft_samp_buf(in_size_val));
}

LTE_fdd_dl_ft_samp_buf::LTE_fdd_dl_ft_samp_buf(size_t in_size_val)
                      ...
                      gr::io_signature::make(MIN_OUT, MAX_OUT, sizeof(int8)))
{
    ...
    {
        in_size = LTE_FDD_DL_FT_IN_SIZE_GR_COMPLEX;
    }else if(in_size_val == sizeof(int8)){
        in_size = LTE_FDD_DL_FT_IN_SIZE_INT8;
    }else{
        in_size = LTE_FDD_DL_FT_IN_SIZE_INT8;
    }
    ...
    i_buf           = (float *)malloc(LTE_FDD_DL_FT_SAMP_BUF_SIZE*sizeof(float));
    q_buf           = (float *)malloc(LTE_FDD_DL_FT_SAMP_BUF_SIZE*sizeof(float));
    samp_buf_w_idx  = 0;
    samp_buf_r_idx  = 0;
    ...
    HEALTH 		= 100;
    TRACKING 		= LIBLTE_SUCCESS;
    lost_frames	   	= 0;
    total_frames	= 0; 
    lost_frames_health  = 0;
    total_frames_health = 0;
    TIMING_OFFSET 	= 0;
    FREQUENCY_OFFSET    = 0;
}

LTE_fdd_dl_ft_samp_buf::~LTE_fdd_dl_ft_samp_buf()
{
    // Print tracking results
    printf("\n***************************** TRACKING COMPLETED *****************************\n");
    if (total_frames>0) {
	float quality = (float)(total_frames - lost_frames)/total_frames*100;
    	printf(">>\tCellID %d\t\tFrames %d\t\tMissing %d\t\tCorrect %d\t\tQuality %0.1f%\n",N_id_cell,total_frames,lost_frames,total_frames-lost_frames, quality);
    } else {
    	printf(">>\tNo LTE cells found\n");
    }
    printf("********************************************************************************\n");
...
int32 LTE_fdd_dl_ft_samp_buf::work(int32                      ninput_items,
                                   ...
                                   gr_vector_void_star       &output_items)
{
    ...
    if(LTE_FDD_DL_FT_IN_SIZE_INT8 == in_size)
    {
        if(samp_buf_w_idx < (LTE_FDD_DL_FT_SAMP_BUF_NUM_FRAMES*phy_struct->N_samps_per_frame - ((ninput_items+1)/2)))
        {
    ...
    }else{ // LTE_FDD_DL_FT_IN_SIZE_GR_COMPLEX == in_size
        if(samp_buf_w_idx < (LTE_FDD_DL_FT_SAMP_BUF_NUM_FRAMES*phy_struct->N_samps_per_frame - (ninput_items+1)))
        {
            copy_input_to_samp_buf(input_items, ninput_items);
    ...
    if(process_samples)
    {
        if(LTE_FDD_DL_FT_SAMP_BUF_STATE_COARSE_TIMING_SEARCH != state )
        {
            // Correct frequency error
            freq_shift(0, LTE_FDD_DL_FT_SAMP_BUF_SIZE, timing_struct.freq_offset[corr_peak_idx]);
        }
        ...
        while(samp_buf_r_idx < (samp_buf_w_idx - num_samps_needed))
        {
            ...
            {
            case LTE_FDD_DL_FT_SAMP_BUF_STATE_COARSE_TIMING_SEARCH:
		libgraph_print_screen(state,NULL,0);
                                                                                      ...
                                                                                      &timing_struct))
                {
                    if(corr_peak_idx < timing_struct.n_corr_peaks)
                    {
                        ...
                        state            = LTE_FDD_DL_FT_SAMP_BUF_STATE_PSS_AND_FINE_TIMING_SEARCH;
                        num_samps_needed = phy_struct->N_samps_per_subfr * PSS_AND_FINE_TIMING_SEARCH_NUM_SUBFRAMES;
                    }else{
		...
		if(LIBLTE_SUCCESS == liblte_phy_find_pss_and_fine_timing(phy_struct,
                                                                         ...
                                                                         &freq_offset))
                {
                    ...
                    {
                        freq_shift(0, LTE_FDD_DL_FT_SAMP_BUF_SIZE, freq_offset);
                        timing_struct.freq_offset[corr_peak_idx] += freq_offset;
                    }

                    // Search for SSS
                    state            = LTE_FDD_DL_FT_SAMP_BUF_STATE_SSS_SEARCH;
                    num_samps_needed = phy_struct->N_samps_per_subfr * SSS_SEARCH_NUM_SUBFRAMES;
                }else{
                    // Go back to coarse timing search
                    state             = LTE_FDD_DL_FT_SAMP_BUF_STATE_COARSE_TIMING_SEARCH;
                    samp_buf_r_idx   += phy_struct->N_samps_per_subfr * COARSE_TIMING_SEARCH_NUM_SUBFRAMES;
                    num_samps_needed  = phy_struct->N_samps_per_subfr * COARSE_TIMING_SEARCH_NUM_SUBFRAMES;
                }
                break;
		...
		if(LIBLTE_SUCCESS == liblte_phy_find_sss(phy_struct,
                                                         ...
                                                         &frame_start_idx))
                {
                    if(is_banned_cell(3*N_id_1 + N_id_2) || !is_forced_cell(3*N_id_1 + N_id_2))
                    {
                        // Go back to coarse timing search
                        state = LTE_FDD_DL_FT_SAMP_BUF_STATE_COARSE_TIMING_SEARCH;
                    	samp_buf_r_idx   += phy_struct->N_samps_per_subfr * COARSE_TIMING_SEARCH_NUM_SUBFRAMES; //
                        ...
                        // Decode BCH
                        state = LTE_FDD_DL_FT_SAMP_BUF_STATE_BCH_DECODE;
		...
		if(LIBLTE_SUCCESS == liblte_phy_get_dl_subframe_and_ce(phy_struct,
                                                                    ...
                                                                    &mib))
                {
                    ...
                    //print_mib(&mib);
		    libgraph_print_screen(state,sfn,N_id_cell,phy_struct->N_rb_dl,N_ant,mib.dl_bw,0,0,0,NULL,total_frames,lost_frames,(char *)"[+] MIB correctly decoded",0);

                    // Decode PDSCH for SIB1
                    state = LTE_FDD_DL_FT_SAMP_BUF_STATE_PDSCH_DECODE_SIB1;
                    if((sfn % 2) != 0)
                    {
                ...
                break;
            case LTE_FDD_DL_FT_SAMP_BUF_STATE_PDSCH_DECODE_SIB1:
		libgraph_print_screen(state,sfn,N_id_cell,phy_struct->N_rb_dl,N_ant,mib.dl_bw,0,0,0,NULL,total_frames,lost_frames,NULL,0);
                if(LIBLTE_SUCCESS == liblte_phy_get_dl_subframe_and_ce(phy_struct,
                                                                       i_buf,
                                                                       q_buf,
                                                                       samp_buf_r_idx,
                                                                      ...
                                                                      &bcch_dlsch_msg))
                {
                    ...
                    // Decode all PDSCHs
                    // state            = LTE_FDD_DL_FT_SAMP_BUF_STATE_PDSCH_DECODE_SI_GENERIC;
                    // N_sfr            = 0;
                    // num_samps_needed = phy_struct->N_samps_per_frame * PDSCH_DECODE_SI_GENERIC_NUM_FRAMES;

		    // Goto CHANNEL ESTIMATION STATE
		    state            = LTE_FDD_DL_FT_SAMP_BUF_STATE_CHANNEL_ESTIMATION;
		    liblte_phy_channel_extender(phy_struct,mib.dl_bw);
		    //N_sfr             = 0;
		    samp_buf_r_idx   += phy_struct->N_samps_per_frame;
		    num_samps_needed  = phy_struct->N_samps_per_frame;
                }else{
		...
		// Timing offset tracking & correction
		TRACKING = liblte_phy_timing_tracking_rs(phy_struct,i_buf,q_buf,samp_buf_r_idx,N_id_cell,N_ant,&subframe,5,&timing_offset,sfn_offset,rrc_msg,mib);
		if(TRACKING != LIBLTE_SUCCESS)
        {	
			TRACKING = liblte_phy_timing_tracking_mib(phy_struct,i_buf,q_buf,samp_buf_r_idx,N_id_cell,N_ant,sfn_offset,rrc_msg,mib,10,&timing_offset);
		}
		samp_buf_r_idx += timing_offset;
		TIMING_OFFSET  += (float)timing_offset*1000000000/liblte_phy_fs_num[fs];

		// Frequency offsets tracking & correction
		liblte_phy_frequency_tracking_cp(phy_struct,i_buf,q_buf,samp_buf_r_idx,&freq_offset);
		freq_shift(samp_buf_r_idx,phy_struct->N_samps_per_frame, freq_offset);
        FREQUENCY_OFFSET = timing_struct.freq_offset[corr_peak_idx] + freq_offset;
		
		if(TRACKING == LIBLTE_SUCCESS)
        {		
			// PSS Channel Estimation
			//liblte_phy_pss_ce(sfn,phy_struct,i_buf,q_buf,N_id_2,samp_buf_r_idx,N_id_cell);

			// SSS Channel Estimation	
			//liblte_phy_sss_ce(sfn,phy_struct,i_buf,q_buf,N_id_1,N_id_2,samp_buf_r_idx,N_id_cell);

			// RS Channel Estimation
			liblte_phy_rs_ce(sfn,phy_struct,i_buf,q_buf,samp_buf_r_idx,N_id_cell,N_ant,&ant_power[0],&subframe);
		} else {
			lost_frames++;
			lost_frames_health++;	
		}				
		
		if (sfn % HEALTH_UPDATE == 0) {
			HEALTH = (float)(total_frames_health-lost_frames_health)/total_frames_health*100;
			total_frames_health = 0;
			lost_frames_health  = 0;
			libgraph_print_screen(state,sfn,N_id_cell,phy_struct->N_rb_dl,N_ant,mib.dl_bw,HEALTH,TIMING_OFFSET,FREQUENCY_OFFSET,&ant_power[0],total_frames,lost_frames,NULL,0);
		}

		if (HEALTH <= HEALTH_THRESHOLD) {
            // Go back to coarse timing search
			libgraph_print_screen(state,sfn,N_id_cell,phy_struct->N_rb_dl,N_ant,mib.dl_bw,HEALTH,TIMING_OFFSET,FREQUENCY_OFFSET,&ant_power[0],total_frames,lost_frames,(char *)"[!] Signal Health too low - Going back to COARSE SEARCH",3);
			sfn 			= 0;
			HEALTH 			= 100;
			TIMING_OFFSET 	   	= 0;
			FREQUENCY_OFFSET     	= 0;
...
void LTE_fdd_dl_ft_samp_buf::init(void)
{
    state                   = LTE_FDD_DL_FT_SAMP_BUF_STATE_COARSE_TIMING_SEARCH;
    phich_res               = 0;
    sfn                     = 0;
...
void LTE_fdd_dl_ft_samp_buf::copy_input_to_samp_buf(gr_vector_const_void_star &input_items, int32 ninput_items)
{
...
void LTE_fdd_dl_ft_samp_buf::freq_shift(uint32 start_idx, uint32 num_samps, float freq_offset)
{
    float  f_samp_re;
    float  f_samp_im;
    float  tmp_i;
...
void LTE_fdd_dl_ft_samp_buf::print_mib(LIBLTE_RRC_MIB_STRUCT *mib)
{
...
void LTE_fdd_dl_ft_samp_buf::print_sib1(LIBLTE_RRC_SYS_INFO_BLOCK_TYPE_1_STRUCT *sib1)
{
    uint32 i;
    uint32 j;
    uint32 si_win_len;
...
void LTE_fdd_dl_ft_samp_buf::print_sib2(LIBLTE_RRC_SYS_INFO_BLOCK_TYPE_2_STRUCT *sib2)
{
    uint32 coeff = 0;
    uint32 T     = 0;
    uint32 i;
...
void LTE_fdd_dl_ft_samp_buf::print_sib3(LIBLTE_RRC_SYS_INFO_BLOCK_TYPE_3_STRUCT *sib3)
{
...
void LTE_fdd_dl_ft_samp_buf::print_sib4(LIBLTE_RRC_SYS_INFO_BLOCK_TYPE_4_STRUCT *sib4)
{
    uint32 i;
    uint32 stop;
...
void LTE_fdd_dl_ft_samp_buf::print_sib5(LIBLTE_RRC_SYS_INFO_BLOCK_TYPE_5_STRUCT *sib5)
{
    uint32 i;
    uint32 j;
    uint16 stop;
...
void LTE_fdd_dl_ft_samp_buf::print_sib6(LIBLTE_RRC_SYS_INFO_BLOCK_TYPE_6_STRUCT *sib6)
{
    uint32 i;
...
void LTE_fdd_dl_ft_samp_buf::print_sib7(LIBLTE_RRC_SYS_INFO_BLOCK_TYPE_7_STRUCT *sib7)
{
    uint32 i;
    uint32 j;
...
void LTE_fdd_dl_ft_samp_buf::print_sib8(LIBLTE_RRC_SYS_INFO_BLOCK_TYPE_8_STRUCT *sib8)
{
    uint32 i;
    uint32 j;
    uint32 k;
...
void LTE_fdd_dl_ft_samp_buf::print_sib13(LIBLTE_RRC_SYS_INFO_BLOCK_TYPE_13_STRUCT *sib13)
{
    uint32 i;
...
void LTE_fdd_dl_ft_samp_buf::print_page(LIBLTE_RRC_PAGING_STRUCT *page)
{
    uint32 i;
    uint32 j;
...
void LTE_fdd_dl_ft_samp_buf::print_config(void)
{
    uint32 i;

    printf("***System Configuration Parameters***\n");
    printf("\tType 'help' to reprint this menu\n");
    printf("\tType 'quit' to close the software\n");
    ...
    // BANNED CELLS
    printf("\t%-30s = %5s", BANNED_CELLS_PARAM,"");
    for(i=0; i<n_banned_cells; i++)
    {
        ...
        printf("%d", banned_cells[i]);
    }
    printf(", values = [0 - 268435455]\n");

    // FORCED CELLS
    printf("\t%-30s = %5s", FORCED_CELLS_PARAM,"");
    for(i=0; i<n_forced_cells; i++)
    {
        ...
        printf("%d", forced_cells[i]);
    }
    printf(", values = [0 - 268435455]\n");
}

void LTE_fdd_dl_ft_samp_buf::change_config(char *line)
{
    char *param;
    char *value;
    bool  err = false;
	...
	if(!strcasecmp(param, "quit"))
	{
	    exit(0);
	}else if(!strcasecmp(param, "help")) {
            print_config();
        }else if(value != NULL){
            if(!strcasecmp(param, FS_PARAM)) {
                err = set_fs(value);
            } else if (!strcasecmp(param, BANNED_CELLS_PARAM)) {
                err = set_banned_cells(value);
            } else if (!strcasecmp(param, FORCED_CELLS_PARAM)) {
                err = set_forced_cells(value);
            } else {
...
bool LTE_fdd_dl_ft_samp_buf::set_fs(char *char_value)
{
    bool err = false;
...
bool LTE_fdd_dl_ft_samp_buf::set_banned_cells(char* banned_cells_list)
{
    bool err = false;
    char * banned_cell;
    uint32 banned_cell_value;

    n_banned_cells = 0;
    banned_cell = strtok(banned_cells_list,","); 
    while ((banned_cell != NULL) && (n_banned_cells<=16)) {
	banned_cell_value = atoi(banned_cell);

	if ((banned_cell_value >= 0) && (banned_cell_value <= 268435455)) {
		banned_cells[n_banned_cells] = banned_cell_value;
		n_banned_cells++;
		banned_cell = strtok(NULL,",");
	} else {
		err = true;
		break;
	} 
...
bool LTE_fdd_dl_ft_samp_buf::is_banned_cell(uint32 cell_to_verify)
{
    bool banned = false;
...
bool LTE_fdd_dl_ft_samp_buf::set_forced_cells(char* forced_cells_list)
{
    bool err = false;
    char * forced_cell;
    uint32 forced_cell_value;

    n_forced_cells = 0;
    forced_cell = strtok(forced_cells_list,","); 
    while ((forced_cell != NULL) && (n_forced_cells<=16)) {
	forced_cell_value = atoi(forced_cell);

	if ((forced_cell_value >= 0) && (forced_cell_value <= 268435455)) {
		forced_cells[n_forced_cells] = forced_cell_value;
		n_forced_cells++;
		forced_cell = strtok(NULL,",");
	} else {
		err = true;
		break;
	} 
...
bool LTE_fdd_dl_ft_samp_buf::is_forced_cell(uint32 cell_to_verify)
{
    if (n_forced_cells == 0)
    {
	return(true);
